...
`scripts/self_explore_room.py` is a wall-following room exploration proof. It maintains a simple occupancy grid from TOF rays, records encoder odometry, and stops when it has not discovered new grid cells for a sustained period. It runs a continuous control loop: every tick reads cached TOF values, updates odometry, adds rays to the map, and steers the left/right motors.
